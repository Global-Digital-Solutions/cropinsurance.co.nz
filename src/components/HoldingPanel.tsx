export default function HoldingPanel() {
  return (
    <div className="bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden max-w-lg w-full mx-auto">
      {/* Header */}
      <div className="bg-gradient-to-r from-green-800 to-green-700 px-5 sm:px-8 py-6">
        <h2 className="text-white text-xl font-semibold leading-snug">
          Find a crop insurance specialist
        </h2>
        <p className="text-green-100 text-sm mt-2 leading-relaxed">
          Crop and horticultural cover is a specialist area with a small number of providers. Use the options below to find the right adviser or insurer for your operation.
        </p>
      </div>
      ...
      <div className="px-5 sm:px-8 py-6 space-y-3">
        ...
        <a
          ...
          className="flex items-start gap-4 p-4 rounded-xl border border-slate-200 hover:border-green-300 hover:bg-green-50 transition-colors group"
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-slate-500 text-xs mt-0.5">Search IBANZ, the NZ brokers association, to find an adviser who specialises in agricultural and horticultural cover.</p>
          </div>
          <svg className="w-4 h-4 text-slate-300 group-hover:text-green-500 flex-shrink-0 mt-1 ml-auto transition-colors" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
        ...
        <div className="flex items-start gap-4 p-4 rounded-xl border border-slate-200">
          ...
          <div>
            ...
            <p className="text-slate-500 text-xs mt-0.5">Some crop insurers deal directly with growers. Visit their websites to enquire or get a quote from their own team.</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-5 sm:px-8 py-5 bg-slate-50 border-t border-slate-100">
        <p className="text-xs font-semibold text-slate-700 mb-1">Are you a crop insurance specialist?</p>
        <p className="text-xs text-slate-500 leading-relaxed">
          If you work in agricultural insurance and cover crops or horticulture, we would like to hear from you.{' '}
          <a href="mailto:hello@cover4you.co.nz" className="font-medium text-green-700 hover:underline">hello@cover4you.co.nz</a>
        </p>
      </div>
    </div>
  );
